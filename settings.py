...
class Settings(QDialog):
    ...
    def create_layout(self):
        ...
        layout.addWidget(Dropdown("SDR is on", self.params.sdr_location, VALID_LOCATIONS, LOCAL, self.sdr_location_changed))
        # SDR IP
        self.sdr_ip = TextInput("SDR IP", self.params.sdr_ip, lambda ip: self.params.set_sdr_ip(ip), self.params.sdr_location == NETWORK)
        layout.addWidget(self.sdr_ip)
        # SDR Port
        ...
        self.radio_info = TextInput('Radio Info Port', self.params.radio_info_port, lambda port: self.params.set_radio_info_port(port), self.params.cat_software == N1MM, INTEGER)
    ...
    def sdr_location_changed(self, location):
        self.params.set_sdr_location(location)
        self.sdr_ip.set_enabled(location == NETWORK)
